...
class IntegrationRegistry:
    ...
    def register_provider(self, provider: IntegrationProvider) -> None:
        ...
        logger.info(f"IntegrationRegistry registered provider '{provider.provider_id}' ({provider.name}) with priority {provider.priority}")
    ...
    def find_by_capability(self, category: IntegrationCapability) -> list[IntegrationProvider]:
    ...
    def resolve_active_provider(self, category: IntegrationCapability) -> Optional[IntegrationProvider]:
        ...
        for p in candidates:
            if p.status in [IntegrationStatus.CONNECTED, IntegrationStatus.READY, IntegrationStatus.CONFIGURED]:
                return p
