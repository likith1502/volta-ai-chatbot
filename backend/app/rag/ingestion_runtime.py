...
class IngestionRuntime:
    ...
    async def ingest_document(
        self, document: Document, chunk_size: int = 512, chunk_overlap: int = 64
    ) -> IngestionJob:
        ...
        try:
            ...
            parsed_text = (
                parser.parse(document.raw_text, document.metadata)
                if parser
            ...
            logger.info(
                f"IngestionRuntime successfully ingested document '{document.document_id}' with {len(chunks)} chunks."
            )
            return job

        except Exception as exc:
            logger.error(
                f"IngestionRuntime failed for document '{document.document_id}': {exc}"
            )
            self.job_manager.update_job_status(
                job.job_id, status=JobStatus.FAILED, error=str(exc)
            )
            raise exc
