...
class MemoryFactory:
    ...
    @staticmethod
    def create_repository(config: Optional[MemoryConfiguration] = None) -> MemoryRepository:
        config = config or MemoryConfiguration()
        # Default in-memory reference implementation
        repo = InMemoryMemoryRepository(capacity=config.max_memories_per_conversation * 4)
