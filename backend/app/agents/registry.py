...
class AgentRegistry:
    ...
    def register_definition(self, definition: AgentDefinition) -> None:
        ...
        logger.info(
            f"AgentDefinition '{definition.definition_id}' registered for '{definition.identity.name}'"
        )

    def register_agent(self, agent: Agent) -> None:
        self.repository.save_agent(agent)
        logger.info(
            f"Agent '{agent.agent_id}' ('{agent.name}') registered in AgentRegistry."
        )
    ...
    def find_by_capability(self, capability_name: str) -> list[Agent]:
        ...
        for a in self.list_agents():
            ...
            if (
                getattr(caps, f"supports_{capability_name}", False)
                or capability_name in caps.supported_tools
            ):
                results.append(a)
        return results
