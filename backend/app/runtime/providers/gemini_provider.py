...
class GeminiProvider(RuntimeProvider):
    ...
    def __init__(
        self, api_key: str = "", default_model: str = "gemini-2.5-flash"
    ) -> None:
        self._api_key = (
            api_key
    ...
    async def generate(self, request: RuntimeRequest) -> RuntimeResponse:
        ...
        if not self._api_key or not self._api_key.strip():
            raise ProviderConfigurationError(
                "Gemini API key is not configured in GEMINI_API_KEY or GOOGLE_API_KEY environment variables."
            )
        ...
        try:
            ...
            if hasattr(res, "usage_metadata") and res.usage_metadata:
                prompt_tokens = (
                    getattr(res.usage_metadata, "prompt_token_count", 0) or 0
                )
                completion_tokens = (
                    getattr(res.usage_metadata, "candidates_token_count", 0) or 0
                )
            ...
            estimated_cost = ((prompt_tokens / 1000.0) * cost_per_1k_input) + (
            ...
            if "api_key" in err_msg or "auth" in err_msg or "unauthorized" in err_msg:
                raise ProviderAuthenticationError(
                    f"Gemini API authentication failed: {exc}"
                )
