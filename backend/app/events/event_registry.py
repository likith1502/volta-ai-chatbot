...
class WorkflowEventRegistry:
    ...
    def __init__(self) -> None:
        self._subscriptions: dict[
            str, Union[EventSubscription, Callable[[], WorkflowEventListener]]
        ] = {}
    ...
    def register(
        ...
        if target_id is None:
            ...
            elif isinstance(listener, type) and issubclass(
        ...
        if isinstance(listener, WorkflowEventListener):
            ...
            if isinstance(res, WorkflowEventListener):
                ...
                self._subscriptions[target_id] = sub
                return sub
            raise EventListenerNotFoundError(
                f"Factory function for listener '{target_id}' did not return a WorkflowEventListener."
            )
        raise EventListenerNotFoundError(
            f"Invalid listener registration type for '{target_id}'."
        )
    ...
    def lookup(self, listener_id: str) -> EventSubscription:
        ...
        if isinstance(sub, EventSubscription):
            return sub
        raise EventListenerNotFoundError(
            f"Invalid subscription stored for '{listener_id}'."
        )
    ...
    def list_subscribers(self) -> List[EventSubscription]:
        ...
        active_subs = [
            ...
            if isinstance(s, EventSubscription) and s.enabled
        ]
        return sorted(
            active_subs, key=lambda s: weights.get(s.priority, 2), reverse=True
        )
