...
from app.deployment.scaling import AutoScalingPolicy, HorizontalScaling, ReplicaPolicy, ResourceLimits, VerticalScaling
...
class DeploymentFactory:
    ...
    def create_default_strategy() -> RollingDeployment:
        return RollingDeployment(config=DeploymentStrategyConfig(max_surge=1, max_unavailable=0))

    @staticmethod
    def create_default_scaler() -> HorizontalScaling:
        return HorizontalScaling(
            policy=ReplicaPolicy(min_replicas=1, max_replicas=10, desired_replicas=2),
            auto_policy=AutoScalingPolicy(cpu_threshold_pct=70.0, memory_threshold_pct=80.0),
        )
