...
class PromptOptimizationResult:
    def __init__(self, original_chars: int, optimized_chars: int) -> None:
        ...
        self.optimization_pct = (
            max(0.0, (1.0 - (optimized_chars / original_chars)) * 100.0)
            if original_chars > 0
            else 0.0
        )
...
class PromptOptimizer:
    ...
    def optimize(self, response: PromptResponse) -> PromptOptimizationResult:
        ...
        for msg in response.messages:
            ...
            cleaned_content = "\n".join(
                line.strip() for line in msg.content.splitlines() if line.strip()
            )
            ...
            if previous_msg and previous_msg.role == msg.role:
                ...
                new_msg = ChatMessage(
                    role=msg.role, content=cleaned_content, metadata=msg.metadata
                )
        ...
        if response.system_prompt:
            response.system_prompt = "\n".join(
                line.strip()
                for line in response.system_prompt.splitlines()
                if line.strip()
            )
        ...
        return PromptOptimizationResult(
            original_chars=original_size, optimized_chars=optimized_size
        )
